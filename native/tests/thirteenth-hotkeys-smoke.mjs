...
const evidence = { date: new Date().toISOString(), exe, data, result: 'pending', checks: [], cleanup: {} };
...
const focus = (owner, title) => { safe(); const window = windowOf(owner, title); input({ op: 'focus', ...owner, hwnd: window.hwnd }); return window; };
const keys = (owner, key, modifiers = 0, repeats = 1, title) => {
  assertExclusive(true); safe(); const window = windowOf(owner, title);
  input({ op: 'keys', ...owner, hwnd: window.hwnd, key, modifiers, repeats });
};
...
const physicalClick = async (page, locator, title, fraction = { x: 0.5, y: 0.5 }) => {
  assertExclusive(true); safe(); await locator.scrollIntoViewIfNeeded(); const box = await locator.boundingBox();
...
try {
  ...
  Object.assign(state.preferences, { autoHide: false, shortcuts: [
    ...
    { id: 'panel-item', code: 'KeyQ', ctrl: false, shift: false, alt: false, scope: 'panel', action: 'item', projectId: 'launcher', itemId: 'app' },
    { id: 'global-project', code: 'KeyP', ctrl: true, shift: true, alt: true, scope: 'global', action: 'project', projectId: 'group' },
    { id: 'panel-edit', code: 'KeyE', ctrl: false, shift: false, alt: false, scope: 'panel', action: 'edit' },
  ] });
  state.projects = [
  ...
  externalChild = spawn(externalExe, [], { windowsHide: true, stdio: 'ignore' });
  ...
  await expect.poll(() => inspect(external).windows.some(w => w.visible && w.title.startsWith('Luma shortcut fixture'))).toBe(true);
  ...
  expect(probe(0x51)).toBe(true);
  await command(externalDir, 'release'); await save(() => {});
  await expect.poll(async () => (await status('global-item'))?.registered).toBe(true);
  check('real RegisterHotKey occupation is visible; release plus save re-registers; panel Q is not globally registered');

  focus(external); keys(external, 0x47, 7, 10);
  ...
  await wait(350); focus(external); keys(external, 0x47, 7);
  await expect.poll(() => inspect(firstTarget).foregroundPid).toBe(firstTarget.pid); expect(await readRows(targetDir)).toHaveLength(1);
  await closeOwned(firstTarget); focus(external);
  const beforeKeys = (await readEvents()).filter(event => event.type === 'key' && event.code === 0x51).length;
  keys(external, 0x51, 0, 3); await wait(500);
  expect((await readEvents()).filter(event => event.type === 'key' && event.code === 0x51).length).toBeGreaterThan(beforeKeys);
  expect(await readRows(targetDir)).toHaveLength(1);
  check('physical global chord launches one dedicated process and reuses its window; external Q remains ordinary textbox input');
  ...
  const collapse = async () => {
    ...
    focus(external); await expect.poll(suspendCount).toBeGreaterThan(before);
  };
  await collapse(); keys(external, 0x50, 7, 10);
  await expect(dock.getByRole('region', { name: '快捷键分组 文件夹堆叠', exact: true })).toBeVisible();
  await wait(400);
  await expect.poll(() => dock.evaluate(() => window.__r13Activations.length)).toBe(1);
  ...
  keys(host, 0x45, 0, 10, 'Luma Dock'); await expect(dock.locator('.dock')).toHaveClass(/dock-editing/); await wait(400);
  expect(await dock.evaluate(() => window.__r13Activations.filter(event => event.id === 'panel-edit').length)).toBe(1);
  await wait(350); keys(host, 0x45, 0, 1, 'Luma Dock'); await expect(dock.locator('.dock')).not.toHaveClass(/dock-editing/);
  keys(host, 0x51, 0, 1, 'Luma Dock');
  await expect.poll(async () => (await readRows(targetDir)).length).toBe(2); await captureTargets();
  const secondTarget = [...targets.values()].at(-1); await expect.poll(() => inspect(secondTarget).foregroundPid).toBe(secondTarget.pid);
  check('physical click grants real dock/document focus; held panel E toggles once and panel Q launches one new test instance');
  ...
  await expect.poll(async () => (await status('global-item'))?.registered).toBe(true);
  expect(inspect(host).foregroundPid).toBe(host.pid); expect(await readRows(targetDir)).toHaveLength(2);
  ...
  focus(external); keys(external, 0x47, 7); await wait(500); expect(inspect(external).foregroundPid).toBe(external.pid);
  keys(external, 0x48, 7); await expect.poll(() => inspect(secondTarget).foregroundPid).toBe(secondTarget.pid); expect(await readRows(targetDir)).toHaveLength(2);
  ...
  expect(probe(0x48, 7)).toBe(true); expect(probe(0x50, 7)).toBe(true); expect(probe(0x51)).toBe(true);
