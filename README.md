...
- **文件拖放**：普通模式拖到文件夹后确认复制；Ctrl 复制、Shift 同卷移动、Alt 创建快捷方式。整理模式只添加或合并快捷引用。文件操作同名不覆盖，支持目录和 Windows 下载来源标记。
- **软件入口**：有专属最近来源的软件显示最近项目；其他软件直接提供带软件图标的“打开软件”。菜单标题和名称统一单行，长名称保留完整提示。
- **手势**：单击直达主目录；长按 300ms 展开堆叠、滑选松手打开；右键拖动平移所有子菜单，原地右击打开菜单。
- **外观**：宽高、图标、圆角可调；实色 / 清透 / 强磨砂三种材质（明暗双主题，强磨砂带颗粒感）；动画速度三档，弹出时一级先、二级后，收起时二级先、一级后；切换已打开的菜单直接更新内容，回收期间移回一级或二级菜单均可唤回。
...
- 0.4.6 修正无新鼠标位移时热区重排启动驻留的路径，并补充退出原因诊断；用户报告的自行退出及全部静止误弹原因仍未确认。详情见 [第二十三轮报告](docs/第二十三轮修正与交付报告.md)。
- 文件复制每批最多 100 个顶层来源、总树 200 项/16 层/2 GiB；不覆盖现有文件，不复制重解析点或 EFS，移动仅同卷。
