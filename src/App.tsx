...
export default function App() {
  ...
  return <>
    ...
    {searchOpen && <Modal title="搜索项目与入口" onClose={() => { setSearchOpen(false); setQuery(''); }}><div className="search-input"><Search size={20}/><input autoFocus aria-label="搜索项目和文件夹" placeholder="项目名称、文件夹、路径…" value={query} onChange={e => setQuery(e.target.value)}/><kbd>ESC</kbd></div><div className="search-results">{state.projects.flatMap(project => project.items.filter(item => fuzzyIncludes(query, `${project.name} ${item.name} ${item.path}`)).map(item => <button key={`${project.id}-${item.id}`} onClick={() => { void openItem(project, item); setSearchOpen(false); setQuery(''); }}><CrystalFolder color={project.color} size={30}/><span><strong>{item.name}</strong><small>{project.name} · {item.path}</small></span><ArrowUpRight size={16}/></button>))}{!state.projects.some(p => p.items.some(i => fuzzyIncludes(query, `${p.name} ${i.name} ${i.path}`))) && <p className="empty-message">没有匹配的入口</p>}</div></Modal>}
