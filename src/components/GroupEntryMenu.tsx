...
import { AppWindow, FolderOpen, X, ArrowUpRight } from 'lucide-react';
...
import { appCapabilityKey, useAppCapabilities } from './useAppCapabilities';
...
export function GroupEntryMenu({ project, highlight, filter, badgeForItem, onOpen, onBrowse, editing, onRemove, onExtract, onMoveItem, onUngroup, renderAccessory, ...gesture }: { renderAccessory?: (item: LaunchItem) => ReactNode; filter?: string; badgeForItem?: (itemId: string) => string | undefined; project: Project; highlight: string | null; onOpen: (item: LaunchItem) => void; onBrowse: (item: LaunchItem) => void; editing?: boolean; onRemove?: (itemId: string) => void; onExtract?: (itemId: string) => void; onMoveItem?: (sourceId: string, itemId: string, targetId?: string) => void; onUngroup?: () => void; onPointerDown: (event: PointerEvent<HTMLButtonElement>) => void; onPointerMove: (event: PointerEvent) => void; onPointerUp: (event: PointerEvent) => void; onPointerCancel: () => void }) {
  ...
  const capabilities = useAppCapabilities([project]);
  const canBrowse = (item: LaunchItem) => item.kind === 'folder' || item.kind === 'app' && capabilities.has(appCapabilityKey(project.id, item.id));
  const DefaultIcon = project.items[0]?.kind === 'app' ? AppWindow : FolderOpen;
  ...
  return <div className="folder-column group-column" data-drop-project={project.id} aria-label={`${project.name} 组内入口`} onDragOver={event => { if (editing && event.dataTransfer.types.includes('application/x-luma-entry')) { event.preventDefault(); event.stopPropagation(); } }} onDrop={event => { if (!editing || !event.dataTransfer.types.includes('application/x-luma-entry')) return; event.preventDefault(); event.stopPropagation(); try { const data = JSON.parse(event.dataTransfer.getData('application/x-luma-entry')); onMoveItem?.(data.projectId, data.itemId, project.id); } catch { /* Ignore malformed external drag payloads. */ } }}>
    <div className="stack-items launch-grid" {...pan}>
      {visible.map(item => <div className="group-entry-row" key={item.id} data-transfer-project={item.kind === 'folder' ? project.id : undefined} data-transfer-item={item.kind === 'folder' ? item.id : undefined} data-transfer-name={item.kind === 'folder' ? item.name : undefined} onContextMenu={event => { event.preventDefault(); gesture.onPointerCancel(); setContext({ x: event.clientX, y: event.clientY, actions: [
        { label: '打开', action: () => onOpen(item) },
        ...(canBrowse(item) ? [{ label: item.kind === 'app' ? '最近项目' : '进入子菜单', action: () => onBrowse(item) }] : []),
        ...
        <SplitFolderTile enabled={!editing && ['folder','app'].includes(item.kind)} software={item.kind === 'app'} canEnter={canBrowse(item)} name={item.name} projectId={project.id} entryId={item.id} onOpen={() => onOpen(item)} onEnter={() => { if (canBrowse(item)) onBrowse(item); }} gesture={gesture}>
          <button data-item-id={item.id} data-project-id={project.id} className={`stack-item ${highlight === item.id ? 'selected' : ''}`} title={`${item.name}\n${item.path}`} {...(!editing ? gesture : {})} draggable={editing || item.kind !== 'url'} onDragStart={event => { gesture.onPointerCancel(); event.dataTransfer.effectAllowed = editing ? 'move' : 'all'; event.dataTransfer.setData('application/x-luma-entry', JSON.stringify({ projectId: project.id, itemId: item.id })); }} onLostPointerCapture={gesture.onPointerCancel} onClick={event => { if (!editing && event.detail === 0) onOpen(item); }}>
            <ItemIcon projectId={project.id} itemId={item.id} pathKey={item.path} kind={item.kind} color={project.color} size={38}/><span><strong title={item.name}>{item.name}</strong><small>{item.kind === 'folder' ? '文件夹' : item.kind === 'app' ? '软件' : item.kind === 'url' ? '网站' : '文件'}</small></span>
          </button>
        </SplitFolderTile>
    ...
    <footer><button data-item-id={project.items[0].id} data-project-id={project.id} className="text-button directory-open-current" {...gesture} onLostPointerCapture={gesture.onPointerCancel} onClick={event => { if (event.detail === 0) onOpen(project.items[0]); }}><DefaultIcon size={14}/>打开默认入口</button>{project.items.length > 1 && <button className="text-button directory-open-current" onClick={onUngroup}>拆成独立图标</button>}</footer>
