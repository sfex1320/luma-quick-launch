...
export function Appearance({ value, onChange, preview }: { value: Preferences; onChange: (p: Preferences) => void; preview?: ReactNode }) {
  ...
  return <div className="appearance" aria-label="外观设置">
    ...
    <section className="cp-card">
      ...
      <label className="toggle-row"><span>动画速度<small>子菜单先快速收起，面板随后；展开顺序相反</small></span><select aria-label="动画速度" value={value.motionSpeed ?? 'standard'} onChange={e => change('motionSpeed', e.target.value as Preferences['motionSpeed'])}><option value="relaxed">舒缓</option><option value="standard">标准</option><option value="brisk">快速</option></select></label>
