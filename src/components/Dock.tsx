...
export function Dock({ onFavorite, projects, preferences: p, onOpen, onSettings, onSearch, onDropFiles, onDropText, onGroup, onUngroup, onReorder, onRemove, onMoveItem, overlay = false }: Props) {
  ...
  const showStack = (id: string | null) => { menuGeneration.current++; const project = projects.find(project => project.id === id); setBrowseItemId(project ? remember(project).browse : null); setMenuError(''); setStack(id); setMore(false); };
  const selectBrowse = (id: string | null) => { if (active) remember(active).browse = id; setBrowseItemId(id); };
