...
export function DirectoryActionButtons({ projectId, itemId, listing, controller, highlight, gesture, editing = false, badges }: { projectId: string; itemId: string; listing: FolderListing; controller: Controller; highlight: string | null; gesture: ActionGesture; editing?: boolean; badges?: Map<string, string> }) {
  ...
  return <>
    <button type="button" {...props('create-directory')} className={`text-button directory-open-current ${highlight === actionId('create-directory') ? 'selected' : ''}`} disabled={controller.running} title="在当前实际目录新建文件夹" onClick={event => { if (event.detail === 0) controller.create(listing); }}><FolderPlus size={13}/>新建文件夹{editing && badges?.get('newFolder') && <span className="shortcut-badge">{badges.get('newFolder')}</span>}</button>
    <button type="button" {...props('copy-directory')} className={`text-button directory-open-current ${highlight === actionId('copy-directory') ? 'selected' : ''}`} disabled={controller.running} onClick={event => { if (event.detail === 0) void controller.copy(listing.folderId); }}><Copy size={13}/>复制地址{editing && badges?.get('copyAddress') && <span className="shortcut-badge">{badges.get('copyAddress')}</span>}</button>
