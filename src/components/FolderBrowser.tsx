...
export function FolderBrowser(props: Props) {
  ...
  return <><div ref={root} className="folder-cascade" aria-label="逐级目录">
    {levels.map((level, index) => <FolderPane {...props} key={level.folderId ?? 'root'} level={level} index={index}
      ...
      onBack={index ? () => { props.onPointerCancel(); props.onNavigate?.(); for (const key of listings.current.keys()) if (key >= index) listings.current.delete(key); setLevels(previous => previous.slice(0, index)); } : props.onBackToGroup}/>) }
  </div>{limit && <p className="directory-limit" role="status">{limit}</p>}</>;
}

function FolderPane({ projectId, item, color, highlight, onOpen, onRunTest, runningTest, level, index, onListing, onEnter, onBack, onBackToGroup: _back, onNavigate: _navigate, leadingColumns: _leading, ...gesture }: Props & { level: Level; index: number; onListing: (listing: FolderListing | null) => void; onEnter: (id: string, name: string) => void; onBack?: () => void }) {
  ...
  const manual = testTask?.label === '手动启动';
  return <div className="folder-browser folder-column" data-folder-level={index} aria-label={`${listing?.name ?? level.name} 目录层`}>
    <div className="folder-browser-path">
      ...
      <button className="icon-button" aria-label="刷新目录" disabled={loading} onClick={() => { clearFolderThumbnails(); setReload(n => n + 1); }}><RefreshCw size={14}/></button>
    </div>
    ...
    {listing && <>
      <div className="stack-items directory-items launch-grid" aria-label={`${listing.name} 目录内容`}>
        {listing.entries.map(entry => {
          ...
          return <div className="directory-row" key={entry.id}>{entry.kind === 'folder' ? <SplitFolderTile name={entry.name} projectId={projectId} entryId={entry.id} folderItemId={item.id} onOpen={() => onOpen(entry.id)} onEnter={() => onEnter(entry.id, entry.name)} gesture={gesture}>{button}</SplitFolderTile> : button}</div>;
        })}
      ...
      <footer className="directory-footer"><div className="directory-footer-actions">
        ...
        {testTask && <button data-item-id={testTask.id} data-project-id={projectId} data-folder-item-id={item.id} data-test-task-id={testTask.id} className={`text-button directory-open-current directory-run-test ${highlight === testTask.id ? 'selected' : ''}`} title={`${testTask.label} · ${testTask.command}`} disabled={runningTest}
          {...gesture} onLostPointerCapture={gesture.onPointerCancel} onClick={event => { if (event.detail === 0) onRunTest(testTask.id); }}><Terminal size={14}/>{runningTest ? '正在打开终端…' : manual ? '手动启动' : '运行测试'}</button>}
