...
export function ProjectEditor({ project, onSave, onClose }: { project?: Project; onSave: (p: Project) => void; onClose: () => void }) {
  ...
  return <Modal title={project ? '编辑项目堆叠' : '把新项目放到手边'} onClose={close} wide>
    <form className={dropActive ? 'editor-drop-active' : ''} onDragOver={event => { if (!hasExternalFiles(event.dataTransfer)) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'copy'; setDropActive(true); }} onDragLeave={event => { if (!(event.relatedTarget instanceof Node) || !event.currentTarget.contains(event.relatedTarget)) setDropActive(false); }} onDrop={event => { if (!hasExternalFiles(event.dataTransfer)) return; event.preventDefault(); event.stopPropagation(); setDropActive(false); void appendFiles(Array.from(event.dataTransfer.files)); }} onSubmit={event => { event.preventDefault(); if (busy.current) { setError('正在解析入口，请完成后再保存。'); return; } const latest = currentDraft.current; const candidate = { ...latest, name: latest.name.trim(), items: latest.items.map(item => ({ ...item, name: item.name.trim(), path: item.path.trim() })) }; const parsed = ProjectSchema.safeParse(candidate); if (!parsed.success) { setError('请填写项目名称，以及每个入口的名称和路径。'); return; } alive.current = false; onSave(parsed.data); }}>
      ...
      <div className="editor-folders">{draft.items.map((item, index) => <div className="folder-form" key={item.id}><div><input aria-label={`入口 ${index + 1} 名称`} required maxLength={120} placeholder="文件夹、软件或文件名称" value={item.name} onChange={e => updateItem(index, 'name', e.target.value)}/><input aria-label={`入口 ${index + 1} 路径`} required maxLength={4096} placeholder="D:\Projects\..." value={item.path} onChange={e => updateItem(index, 'path', e.target.value)}/></div><select aria-label={`入口 ${index + 1} 类型`} value={item.kind} onChange={e => setDraft(p => ({ ...p, items: p.items.map((entry, i) => i === index ? { ...entry, kind: e.target.value as LaunchItem['kind'] } : entry) }))}><option value="folder">文件夹</option><option value="app">软件</option><option value="file">文件</option></select><button title={nativeMode ? '选择入口' : '浏览器演示请填写路径'} aria-label={`选择入口 ${index + 1} ${item.kind === 'folder' ? '文件夹' : '文件或软件'}`} type="button" className="icon-button" disabled={picking || !nativeMode} onClick={() => item.kind === 'folder' ? pick(index) : pickFiles(index)}><FolderOpen size={17}/></button>{index > 0 && <button type="button" className="icon-button danger" aria-label={`移除入口 ${index + 1}`} onClick={() => setDraft({ ...draft, items: draft.items.filter((_, i) => i !== index) })}><Trash2 size={16}/></button>}</div>)}</div>
